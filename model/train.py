...
parser.add_argument('--model', default='SimpleLSTM', help="The Model Name")
...
def predict(model, params):
    ...
    with open('data/x_test.pk', 'rb') as f:  # Overwrites any existing file.
        rawdata = f.read()
    data_batch = torch.from_numpy(pickle.loads(rawdata).astype(np.float64))

    # move to GPU if available
    if params.cuda:
        data_batch = data_batch.float().cuda(async=True)
    # fetch the next evaluation batch
    data_batch = Variable(data_batch)

    # compute model output
    output_batch = model(data_batch).cpu().data.numpy()
    output_batch = output_batch.reshape(-1, 2)

    # print('output_batch', type(output_batch), output_batch.shape)

    filepath = os.path.join(args.model_dir, "best_prediction.csv")
    df = pd.DataFrame(output_batch, columns=['s', 'n'])
    df['datetime'] = pd.date_range('2016-12-01 06:00:00', periods=180, freq='5min')
    df.to_csv(filepath, index=None, columns=['datetime','s', 'n'])
...
def train(model, optimizer, loss_fn, dataloader, metrics, params):
    ...
    with tqdm(total=len(dataloader), ascii=True) as t:
        for i, (train_batch, labels_batch) in enumerate(dataloader):
            ...
            # move to GPU if available
            if params.cuda:
                train_batch, labels_batch = train_batch.cuda(async=True), labels_batch.float().cuda(async=True)
...
def train_and_evaluate(model, optimizer, scheduler, loss_fn, 
...
def run(args=None):
    ...
    optimizer = AdamW(model.parameters(), lr=params.learning_rate, weight_decay=10e-5)
